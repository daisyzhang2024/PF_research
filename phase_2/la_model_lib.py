...
la_model_lib.py
================
Reusable functions for LA County sales tax revenue modeling. Same
approach as chmr_model_lib.py, adapted to this dataset's columns:
  target      : "Sales Tax Revenue"
  date column : "date"
  macro cols  : unemp, GasPrice, metro_ridership, CPI_U,
                la_search_sentiment, la_news_sentiment

This file has NO top-level execution -- importing it does not run the
backtest or any forecast. Use run_backtest_la.py to re-evaluate all
target/model combos, or run_forecast_la.py to predict a single future
month with the best combo.

Modeling approach (see run_backtest_la.py docstring for full rationale):
...
DATA_PATH = "combined_LA.csv"
DATE_COL = "date"
TARGET_COL = "Sales Tax Revenue"
N_TEST = 24          # rolling-origin backtest window (months held out, walked forward)
RANDOM_STATE = 42

MACRO_COLS = ["unemp", "GasPrice", "metro_ridership", "CPI_U",
              "la_search_sentiment", "la_news_sentiment"]
...
#    month's macro inputs already be known -- see run_forecast_la.py)
# ---------------------------------------------------------------------
...
def forecast_next_month(df, next_month_features: dict, target_kind="yoy_logdiff",
    ...
    IMPORTANT: target_kind/model_name should match whatever run_backtest_la.py
    found to be the best-performing combo (check la_delevel_results.csv --
    for this dataset it was "yoy_logdiff | RandomForest", the default here).

    next_month_features must supply values for that month for:
      unemp, GasPrice, metro_ridership, CPI_U, la_search_sentiment,
      la_news_sentiment, and 'month' (1-12).
    These must be values you already know/have for that month --
    NOT values you're also trying to forecast (see run_forecast_la.py
